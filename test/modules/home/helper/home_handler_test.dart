...
void main() {
  setupTest();
  testWidgets('home handler test', (WidgetTester tester) async {
    // Build our app and trigger a frame.
    final HomeHandlerTester homeHandlerTester = HomeHandlerTester();
          ...
          return MaterialApp(
            home: Material(child: Builder(builder: (context) {
              ...
              return DraggableScrollableSheet(
                ...
                controller: homeHandlerTester.draggableScrollController,
              );
            })),
    ...
    expect(homeHandlerTester.homeScreenBloc.state is HomeStateSelectedState,
        false);
    homeHandlerTester.onLocationTapped(const LocationViewModel(
        url: "url", rating: 3, title: "title", lat: 123, long: 123));
    await tester.pumpAndSettle(const Duration(milliseconds: 100));
    homeHandlerTester.moveInMap(homeHandlerTester.homeScreenBloc.state);
    expect(
        homeHandlerTester.homeScreenBloc.state is HomeStateSelectedState, true);
  });
}

class HomeHandlerTester extends HomeStateHandler {
  @override
